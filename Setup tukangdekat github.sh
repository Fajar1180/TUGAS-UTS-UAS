...
# =============================================================
echo -e "\n${CYAN}[4/4] Membuat GitHub Project Board...${NC}"

PROJECT_ID=$(gh project create \
  --owner "$(echo $REPO | cut -d'/' -f1)" \
  --title "$PROJECT_NAME" \
  --format json 2>/dev/null | python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('number',''))" 2>/dev/null)

if [[ -n "$PROJECT_ID" ]]; then
  echo -e "  ${GREEN}✓${NC} Project Board dibuat: #$PROJECT_ID"
  echo -e "  ${CYAN}→ Tambahkan issues ke board di: https://github.com/$(echo $REPO | cut -d'/' -f1)/projects/$PROJECT_ID${NC}"
else
  echo -e "  ${YELLOW}⚠${NC} Project board dibuat manual di: https://github.com/${REPO}/projects"
fi
